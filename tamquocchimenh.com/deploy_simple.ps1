# Simple PowerShell deployment script for tamquocchimenh.com website
param(
...
Write-Host "Authentication: Password-based" -ForegroundColor Yellow
Write-Host ""
...
$useSshpass = Test-Command "sshpass"
if (-not $useSshpass) {
    Write-Host "Warning: sshpass not found. Will use interactive password prompts." -ForegroundColor Yellow
    Write-Host "To install sshpass on Windows, you can use:" -ForegroundColor Yellow
    Write-Host "  - WSL with: sudo apt-get install sshpass" -ForegroundColor Yellow
    Write-Host "  - Or use plink from PuTTY suite" -ForegroundColor Yellow
}
...
function Invoke-SshCommand {
    ...
    try {
        if ($useSshpass) {
            $result = sshpass -p "$VpsPassword" ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$VpsUser@$VpsIp" $Command 2>&1
        } else {
...
function Copy-FilesToVps {
    ...
    try {
        if ($useSshpass) {
            $result = sshpass -p "$VpsPassword" scp -r -o ConnectTimeout=10 -o StrictHostKeyChecking=no "$LocalPath" "$VpsUser@$VpsIp`:$RemotePath" 2>&1
        } else {
